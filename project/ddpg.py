...
class CriticNetwork(nn.Module):
    ...
    def forward(self, state, action):
        ...
        # Concatenate state_value and action
        state_action_value = T.cat([state_value, action], dim=1)  # dim=1 is feature dimension

        # Now pass concatenated tensor through fc2
        state_action_value = self.fc2(state_action_value)
        state_action_value = self.bn2(state_action_value)

        # Pass action through separate layer (you may reconsider this as well)
        action_value = F.relu(self.action_value(action))

        # Combine state_action_value and action_value (if needed)
        combined = F.relu(state_action_value + action_value)

        q_value = self.q(combined)
        return q_value
...
class Agent(object):        
    def __init__(self,  alpha,beta,input_dims,tau,env,gamma=0.99, n_actions=2, max_size=1000000, layer1_size=400, layer2_size=300, batch_size=64):
        ...
        self.noise = OActionNoise(mu=np.zeros(n_actions))   
    ...
    def learn(self):
        ...
        for i in range(self.batch_size):
            target.append(reward[i] + self.gamma * critic_value[i]*done[i])
        ...
        action_loss = -self.critic.forward(state, mu)
        action_loss = T.mean(action_loss)
    ...
    def update_network_parameters(self, tau=None):
        ...
        critic_state_dict = dict(self.critic.state_dict())
        actor_state_dict = dict(self.actor.state_dict())
        target_critic_dict = dict(self.target_critic.state_dict())
        target_actor_dict = dict(self.target_actor.state_dict())
